...
func (s *LevelDBStore) WriteBatch(batch Batch, put ...bool) error {
	return s.writeBatchWithOptions(batch, nil, put...)
	/*
		// 检查batch是否为nil
		if batch == nil {
...
// writeBatchWithOptions 内部实现
func (s *LevelDBStore) writeBatchWithOptions(batch Batch, writeOpts *opt.WriteOptions, put ...bool) error {
	// 检查batch是否为nil
	if batch == nil {
		return NewError("batch cannot be nil")
	}
	...
	// 执行批量写入操作
	err := db.Write(ldbBatch, writeOpts)
...
// GetDB 获取底层的 LevelDB 实例
// 注意：这个方法应该谨慎使用，仅用于需要直接操作 LevelDB 的场景
