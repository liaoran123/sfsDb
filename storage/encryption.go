...
func NewEncryptedStoreWrapper(underlyingStore Store, config *EncryptionConfig) (*EncryptedStoreWrapper, error) {
	...
	if len(config.MasterKey) == 0 && config.Password == "" {
	...
	if len(config.MasterKey) == 0 {
		...
		// 使用密码派生密钥
		key = pbkdf2.Key([]byte(config.Password), salt, iterations, 32, sha256.New)

		// 创建新的 config 保存完整信息（避免修改原 config）
		finalConfig = &EncryptionConfig{
...
// evictLRU 驱逐最久未使用的缓存条目
func (es *EncryptedStoreWrapper) evictLRU() {
	currentSize := atomic.LoadInt64(&es.cacheSize)
	if currentSize < int64(es.maxCacheEntries) {
		return
	}
...
func (es *EncryptedStoreWrapper) Delete(key []byte) error {
	...
	// 从缓存删除
	cacheKey := string(key)
	if _, loaded := es.decryptionCache.LoadAndDelete(cacheKey); loaded {
		atomic.AddInt64(&es.cacheSize, -1)
	}
...
func (es *EncryptedStoreWrapper) WriteBatch(batch Batch, writeOpts ...*opt.WriteOptions) error {
	...
	// 执行批量操作
	err := es.underlyingStore.WriteBatch(underlyingBatch, writeOpts...)
	if err != nil {
		return err
	}
