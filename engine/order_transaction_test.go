...
func initTestProducts(table *Table, t *testing.T) {
	// 创建测试产品
...
func testOrderCreationWithInventory(productTable, orderTable *Table, t *testing.T) error {
	// 获取初始库存
...
func testOrderPayment(productTable, orderTable *Table, t *testing.T) error {
	// 创建测试账户表
...
func testOrderCancellation(productTable, orderTable *Table, t *testing.T) error {
	// 创建订单
...
func testConcurrentOrderCreation(productTable, orderTable *Table, t *testing.T) error {
	// 重置库存
...
func testInsufficientInventory(productTable, orderTable *Table, t *testing.T) error {
	// 设置低库存
...
// createOrder 创建订单
func createOrder(productTable, orderTable *Table, productID, userID, orderID string, quantity int, t *testing.T) (string, error) {
	// 尝试多次执行，处理并发冲突
	...
	for i := 0; i < maxRetries; i++ {
		...
		// 添加产品表到事务管理器（使用Serializable隔离级别）
		options := DefaultTransactionOptions()
		options.IsolationLevel = Serializable
		productTx, err := productTable.BeginWithBatchAndOptions(batch, options)
		if err != nil {
			return "", fmt.Errorf("创建产品表事务失败: %v", err)
		}

		// 将产品事务添加到事务管理器
		tm.transactions = append(tm.transactions, productTx)

		// 添加订单表到事务管理器（使用Serializable隔离级别）
		orderTx, err := orderTable.BeginWithBatchAndOptions(batch, options)
		if err != nil {
			return "", fmt.Errorf("创建订单表事务失败: %v", err)
		}

		// 将订单事务添加到事务管理器
		tm.transactions = append(tm.transactions, orderTx)
		...
		// 解析产品记录
		txRecord := txIter.Value()
		pk := productTable.GetPrimaryKey()
		txFieldsBytes, err := pk.Parse(productTable.fieldsid, txRecord)
		if err != nil {
			tm.Rollback()
			time.Sleep(time.Millisecond * time.Duration(i+1) * 100) // 增加延迟时间
			continue
		...
		// 获取当前版本号
		currentVersion, ok := (*txProductMap)["v"]
		if !ok {
			tm.Rollback()
			return "", fmt.Errorf("产品缺少版本号字段")
		}
...
// payOrder 支付订单
func payOrder(orderTable, accountTable *Table, orderID, userID string, t *testing.T) error {
	// 获取订单信息
	orderAmount := float64(getOrderAmount(orderTable, orderID, t))

	// 开始账户表事务
	accountTx, err := accountTable.Begin()
	if err != nil {
...
// cancelOrder 取消订单
func cancelOrder(productTable, orderTable *Table, orderID string, t *testing.T) error {
	// 获取订单信息
	...
	// 开始产品表事务
	productTx, err := productTable.Begin()
	if err != nil {
...
func testOrderTransactionRetry(productTable, orderTable *Table, t *testing.T) error {
	// 获取初始库存
	initialStock := getProductStock(productTable, "1", t)
	quantity := 5
	...
	// 创建测试账户表用于支付测试
	accountTable, err := TableNew("accounts_retry")
	if err != nil {
		return fmt.Errorf("创建账户表失败: %v", err)
	}
	accountTable.SetFields(map[string]any{"id": "", "balance": 0.0})
...
func createOrderWithRetry(productTable, orderTable *Table, productID, userID, orderID string, quantity int, t *testing.T) (string, error) {
	// 使用自定义重试配置
	options := DefaultTransactionOptions()
	...
	record := iter.Value()
	pk := productTable.GetPrimaryKey()
	fieldsBytes, err := pk.Parse(productTable.fieldsid, record)
	if err != nil {
		return "", fmt.Errorf("解析产品记录失败: %v", err)
	}

	// 转换为map[string]any
	productMap := productTable.RecordByteToAny(fieldsBytes)
	if productMap == nil {
		return "", fmt.Errorf("转换产品记录失败")
	}
...
func payOrderWithRetry(orderTable, accountTable *Table, orderID, userID string, t *testing.T) error {
	// 使用自定义重试配置
	options := DefaultTransactionOptions()
	options.MaxRetries = 5
	options.InitialRetryDelay = 5 * time.Millisecond
	...
	// 开始订单表事务
	orderTx, err := orderTable.BeginWithOptions(options)
	if err != nil {
		return fmt.Errorf("开始订单事务失败: %v", err)
	}
	...
	// 开始账户表事务
	accountTx, err := accountTable.BeginWithOptions(options)
	if err != nil {
		return fmt.Errorf("开始账户事务失败: %v", err)
	}
...
func getProductStock(table *Table, productID string, t *testing.T) int {
	// 搜索产品
...
func getProductStockInTransaction(tx Transaction, productID string) (int, error) {
	// 搜索产品
	fields := map[string]any{"id": productID}
...
func getOrderAmount(table *Table, orderID string, t *testing.T) int {
	// 搜索订单
...
// getOrder 获取订单信息
func getOrder(table *Table, orderID string, t *testing.T) map[string]any {
	// 搜索订单
	fields := map[string]any{"id": orderID}
	iter, err := table.Search(&fields)
	if err != nil {
...
func resetProductStock(table *Table, productID string, stock int, t *testing.T) {
	// 开始事务
	tx, err := table.Begin()
	if err != nil {
...
func getAccountBalance(table *Table, accountID string, t *testing.T) float64 {
	// 搜索账户
...
func getAccountBalanceInTransaction(tx Transaction, accountID string) (float64, error) {
	// 搜索账户
	fields := map[string]any{"id": accountID}
	iter, err := tx.Search(&fields)
	if err != nil {
...
// getTableFromTransaction 从事务中获取表实例
func getTableFromTransaction(tx Transaction) *Table {
	// 尝试类型断言
	if _, ok := tx.(*TableTransaction); ok {
		// 这里需要一个方法来获取Table实例
		// 由于TableTransaction是Table的内部类型，我们可能需要修改代码来支持这个
		// 为了测试目的，我们暂时返回nil
		return nil
	}
	return nil
