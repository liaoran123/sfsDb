...
func (t *Table) MaxAutoValue() int {
	...
	rkey := key[len(t.primaryKey.Prefix(t.name))+1:]
	var target any
	// 如果主键字段为空，默认使用"id"
...
func (t *Table) Operation(fieldsBytes map[string][]byte, record []byte, batch storage.Batch, dataContainer DataContainer, existFields ...string) {
	_, isPut := dataContainer.(*PutBatchContainer)
	if isPut { //仅当put操作时
	...
	for _, Normal := range t.normalIndex {
		if isPut { //仅当put操作时
			dataContainer.SetValue(t.primaryKey.GetID(&fieldsBytes, existFields...))
		}
...
func (t *Table) ReadByBytes(key []byte) []byte {
	pfx := t.primaryKey.Prefix(t.name)
...
func (t *Table) For() storage.Iterator {
	pfx := t.name + SPLIT
	return t.kvStore.Iterator([]byte(pfx))
}
...
func (t *Table) Search(fields *map[string]any) (*TableIter, error) {
	...
	if idx != nil {
		...
		//没有索引，则设置为全表主键扫描迭代器
		idx = t.primaryKey
		key = t.primaryKey.Prefix(t.name)

		/*
			由于数据库定位全文索引，故不支持无索引的搜索

		return nil, fmt.Errorf("表 '%s'没有匹配的'%s'索引", field[0], t.name)*/
	}
	iter := t.kvStore.Iterator(key)
