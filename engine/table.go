...
func TableNew(name string) (*Table, error) {
	...
	tb.fieldTypeLen = make(map[string]uint8)
...
func (t *Table) GetfieldTypeLen(fieldTypeLens ...*map[string]uint8) *map[string]uint8 {
	...
	flen := 0
	for _, n := range t.GetPrimaryFields() {
		if val, ok := (t.fields[n]); ok {
			//固定长度类型，系统字段提取，字符串类型需要注册指定长度util.RegisterTypeSize。
			flen = util.TypeSize(val)
			t.fieldTypeLen[n] = uint8(flen)
		}
	}
	// 处理动态类型字段
...
// 重置fieldTypeLen
func (t *Table) ResetFieldTypeLen() {
	t.fieldTypeLen = make(map[string]uint8)
}
