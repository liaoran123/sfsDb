...
func TableNew(name string) (*Table, error) {
	...
	tb.id = tb.getSysNameId(name, true) //tb.getSysId("sys-tbid")
	//tb.setSysId("sys-tb-id", tb.id+1)
	return tb, nil
}
...
// 获取或创建一个表或索引的ID
func (t *Table) getSysNameId(name string, istb bool) (r uint8) {
	pfx := "tb"
	if !istb {
		pfx = "idx"
	}
...
func (t *Table) CreateIndex(index Index) error {
	idxid := t.getSysNameId(index.Name(), false)
	err := t.indexs.createIndex(index, idxid)
	if err != nil {
		return err
	}
	//t.setSysId("sys-idx-id", idxid+1)
	return nil
}
...
func (t *Table) Escape(b []byte) []byte {
	if len(b) == 0 || len(t.fields) == 0 {
		return b
	}

	// 预编译字段信息，按长度降序排序，避免嵌套替换问题
	...
	for field := range t.fields {
		pattern := []byte(field)
		replacement := []byte(" " + field + " ")
		fields = append(fields, fieldInfo{pattern, replacement})
	}

	// 按模式长度降序排序
	sort.Slice(fields, func(i, j int) bool {
		return len(fields[i].pattern) > len(fields[j].pattern)
	})

	// 使用 bytes.Buffer 构建结果，预分配足够空间
	var buf bytes.Buffer
	buf.Grow(len(b) * 2)
	...
	for i := 0; i < len(b); {
		...
		for _, field := range fields {
			if i+len(field.pattern) <= len(b) && bytes.Equal(b[i:i+len(field.pattern)], field.pattern) {
				// 找到匹配的字段名，替换为转义后的形式
				buf.Write(field.replacement)
				i += len(field.pattern)
				matched = true
				break
			}
...
// 需要转义的字段
// 字符串类型的字段和-1需要转义。-1是json字符串的key，需要转义
func (t *Table) GetEscapeField(field string) bool {
	typ, exist := t.GetField(field)
	if exist {
		typE := util.GetTypeEnum(typ)
		if typE == util.TypeString || field == "-1" {
			return true
		}
	}
	return false
