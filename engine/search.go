...
// Search 搜索记录
func (s *SearchImpl) Search(fields *map[string]any, ops ...util.ComparisonOperator) (*TableIter, error) {
	return s.Searchs(nil, fields, ops...)
}

// Searchs 搜索记录
func (s *SearchImpl) Searchs(funIter storage.FunIter, fields *map[string]any, ops ...util.ComparisonOperator) (*TableIter, error) {
	...
	if idx != nil {
		fieldsBytes = s.table.FieldsToBytesNil(fields)
	...
	if funIter == nil {
		funIter = s.table.kvStore.Iterator
	}
...
func (s *SearchImpl) SearchRange(funIter storage.FunIter, Start, Limit *map[string]any) (*TableIter, error) {
	...
	idx := s.table.MatchIndexCached(fieldname)
	if idx == nil {
		return nil, fmt.Errorf("字段 '%s' 不存在于表 '%s'", fieldname, s.table.name)
	}
	var Startkey []byte
	var Limitkey []byte
	var fieldsBytes *map[string][]byte

	fieldsBytes = s.table.FieldsToBytesNil(Start)
	Startkey = idx.JoinValue(fieldsBytes, s.table.id)
	fieldsBytes = s.table.FieldsToBytesNil(Limit)
	Limitkey = idx.JoinValue(fieldsBytes, s.table.id)
	// 使用完后将 fieldsBytes 放回对象池
	defer func() {
		if fieldsBytes != nil && *fieldsBytes != nil {
			GlobalFieldsBytesPool.Put(*fieldsBytes)
...
func (s *SearchImpl) BatchRead(records []*map[string]any) (map[any][]byte, error) {
	...
	for _, fields := range records {
		...
		searchImpl := NewSearchImpl(s.table)
...
func (s *SearchImpl) BatchSearch(records []*map[string]any) (map[any]*TableIter, error) {
	...
	for _, fields := range records {
		...
		// 搜索记录
		tbiter, err := searchImpl.Search(fields)
		if err != nil {
			GlobalSearchImplPool.Put(searchImpl)
			return nil, err
		}
