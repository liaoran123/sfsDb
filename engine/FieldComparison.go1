...
type FieldComparison struct {
	...
	value       any                         // 比较值
	innerValues anymatch.Values             // 内部使用的 Values.Match 实例
}

// NewFieldComparison 创建一个新的 FieldComparison 实例
func NewFieldComparison(fieldName string, op anymatch.ComparisonOperator, value any) *FieldComparison {
	return &FieldComparison{
		fieldName:   fieldName,
		op:          op,
		value:       value,
		innerValues: anymatch.NewValues(value),
	}
}
...
func (um *FieldComparison) Match(fields *map[string]any) bool {
	...
	// 创建一个新的 Values.Match 实例，使用字段值作为比较的基础
	fieldMatch := anymatch.NewValues(fieldValue)
	// 使用正确的比较顺序：fieldValue 与 um.value 进行比较
	return fieldMatch.Compare(um.op, um.value)
}

// FieldMatch 是一个更简单的匹配器，用于直接比较字段值
// 它实现了 Match 接口，内部使用 Values.Match 进行比较
