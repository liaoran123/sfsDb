...
func (t *TableIter) ParseBytes(k, v []byte) *map[string][]byte {
	...
	case FullTextIndex:
		...
		fieldsBytes = t.index.Parse(t.table.GetPrimaryKey().GetFields(), k)
	case PrimaryKey:
		fieldsBytes = t.index.Parse(nil, v)
	default:
		fieldsBytes = t.index.Parse(t.table.GetPrimaryKey().GetFields(), v)
	}
	return fieldsBytes
}

func (t *TableIter) ParseRecord(k, v []byte) (rd Record) {
	...
	default: //其他二级索引通过Parse得到的是主键ID值，需要回表才能得到记录。
		// 拼接主键前缀和索引值，得到主键key
		pfx := t.table.GetPrimaryKey().JoinValue(fieldsBytes, t.table.id)
		// 回表读取完整记录
		...
		//通过主键解析记录
		trd := t.table.GetPrimaryKey().Parse(nil, byrecord)
		if trd == nil {
			return nil
		}
...
func (t *TableIter) ExportRecord(esc bool, export ExportRecord) {
	//添加锁，防止并发访问
	t.mu.Lock()
	defer t.mu.Unlock()
	if !t.top[esc]() {
	...
	for {
		...
		if t.Match(t.iter.Key(), t.iter.Value(), t.match) {
			rd = Record(t.ParseRecord(t.iter.Key(), t.iter.Value()))
			rd = rd.Select(t.selects...)
			// 将 Record 转换为 map[string]any，然后取地址
			if !export(&rd) {
				return
			}
		}
