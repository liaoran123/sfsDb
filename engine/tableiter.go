...
func (t *TableIter) ParseRecord(k, v []byte) (rd record.Record) {
	...
	default: //其他二级索引通过Parse得到的是主键ID值，需要回表才能得到记录。
		// 拼接主键前缀和索引值，得到主键key
		pfx := t.table.indexs.GetPrimaryKey().JoinValue(fieldsBytes, t.table.name)
		// 回表读取完整记录
		...
		//通过主键解析记录
		trd := t.table.indexs.GetPrimaryKey().Parse(nil, byrecord)
		if trd == nil {
			return nil
		}
...
func (t *TableIter) GerRecords(esc bool, limit ...int) (r record.Records) {
	return t.GerMultiAndJumpRangeRecords(esc, nil, nil, limit...)
}
...
func (t *TableIter) GerMultiRecords(esc bool, match []Match, limit ...int) (r record.Records) {
	return t.GerMultiAndJumpRangeRecords(esc, nil, match, limit...)
}

// 遍历迭代器返回解析后的记录
// 需要跳跃区间时，但不需要匹配Match
func (t *TableIter) GerjumpRangesRecords(esc bool, jumpRanges []storage.Iterator, limit ...int) (r record.Records) {
	return t.GerMultiAndJumpRangeRecords(esc, jumpRanges, nil, limit...)
}
...
func (t *TableIter) GerMultiAndJumpRangeRecords(esc bool, jumpRanges []storage.Iterator, match []Match, limit ...int) (r record.Records) {
	//添加锁，防止并发访问
	t.mu.Lock()
	defer t.mu.Unlock()
	if !t.top[esc]() {
