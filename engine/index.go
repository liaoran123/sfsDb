...
func (dpk *DefaultPrimaryKey) Parse(tablefields []string, pkfieldTypeLen *map[string]uint8, value []byte) (*map[string][]byte, error) { //pkfieldTypeLen无用
	...
	vals := util.Bytes(value).Split()
	if len(vals) != len(tablefields) {
		return nil, errors.New("DefaultPrimaryKey Parse error: value is not valid")
	}
	fieldsBytes := make(map[string][]byte, len(tablefields))
	var val []byte
	for i, fs := range tablefields {
		val = vals[i]
		fieldsBytes[fs] = val[len(fs)+1:]
	}
	return &fieldsBytes, nil
}
*/
...
// 获取字段在value中对应的位置
func getFieldPos(fields []string, value []byte) []fieldidx {
	indexs := make([]fieldidx, 0, len(fields))
	var f []byte
	for _, fit := range fields {
		f = []byte(SPLIT + fit + SPLIT)
		if index := bytes.Index(value, f); index != -1 {
			indexs = append(indexs, fieldidx{field: fit, index: index})
		}
	}
	//因为value中并不按照字段顺序存储，所以需要根据index排序，才能正确解析字段值。
	slices.SortFunc(indexs, func(a, b fieldidx) int {
		if a.index < b.index {
			return -1
