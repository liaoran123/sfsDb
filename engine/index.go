...
func (dpk *DefaultPrimaryKey) GetID(fieldsBytes *map[string][]byte, existFields ...string) []byte {
	...
	for i, fit := range dpk.fields {
		...
		if v, ok := (*fieldsBytes)[fit]; ok {
			if fieldlen > 1 { //组合主键需要转义，否则无法提取其中某个值
				Value.Write(util.Bytes(v).Escape())
			} else {
				Value.Write(v)
			}
			if i < fieldlen-1 {
...
func (dfi *DefaultFullTextIndex) JoinValue(fieldsBytes *map[string][]byte, tbname string, existFields ...string) []byte {
	if v, ok := (*fieldsBytes)[dfi.ftsplit]; ok {
		...
		if len([]rune(string(v))) > int(dfi.ftlen) {
			...
			tval = util.Bytes(tval).Escape() //CombineBytes生成全文索引kv的时候进行了转义，这里也需要转义才能匹配
			(*fieldsBytes)[dfi.ftsplit] = tval
		}
	}
	return dfi.BaseIndex.JoinValue(fieldsBytes, tbname, existFields...)
}
