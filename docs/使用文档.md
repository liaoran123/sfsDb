...
查询性能数据通过 `QueryPerformance` 结构体记录，包含以下字段：
...
性能数据存储在 `TableIter` 对象中，可以通过 `iter.QueryPerformance` 直接访问。
...
fmt.Printf("查询记录数: %d\n", len(records))
fmt.Printf("总扫描次数: %d\n", iter.QueryPerformance.Count)
fmt.Printf("总耗时: %d 纳秒\n", iter.QueryPerformance.TotalTime)
fmt.Printf("使用的索引ID: %d\n", iter.QueryPerformance.Index)
...
// 计算平均查询耗时
if iter.QueryPerformance.Count > 0 {
    avgTime := float64(iter.QueryPerformance.TotalTime) / float64(iter.QueryPerformance.Count)
    fmt.Printf("平均每条记录耗时: %.2f 纳秒\n", avgTime)
}
```

#### 9.6.3 性能数据的自动更新

sfsDb 会在以下操作中自动更新性能数据：
...
1. **性能优化**：通过分析查询耗时，找出性能瓶颈
2. **查询效率对比**：比较不同查询条件或索引策略的性能差异
3. **系统监控**：在生产环境中监控查询性能，及时发现异常
4. **测试验证**：在测试中验证查询性能是否符合预期

#### 9.6.5 注意事项

1. **性能数据的生命周期**：性能数据仅在 `TableIter` 对象的生命周期内有效，调用 `Release()` 后会被重置
2. **性能开销**：性能跟踪本身会带来微小的性能开销，但通常可以忽略不计
3. **并发安全性**：`QueryPerformance` 不是线程安全的，不建议在多线程环境中并发访问
4. **数据单位**：`TotalTime` 以纳秒为单位，便于精确计算
...
// 使用时需要将 AtomicInt 或 AtomicIntDec 转换为 AtomicMap 类型
...
func main() {
    ...
    // 获取删除操作数量
    deleteCount := monitor.AtomicMap(monitor.AtomicIntDec).Get(tableID, indexID)
...
func main() {
    ...
    for _, config := range monitorConfigs {
        ...
        deleteCount := monitor.AtomicMap(monitor.AtomicIntDec).Get(config.tableID, config.indexID)
        netChange := putCount - deleteCount
...
1. **并发安全性**：`AtomicInt` 和 `AtomicIntDec` 内部使用 `atomic.Int64` 保证并发安全，但直接访问 map 本身不是线程安全的，建议使用 `AtomicMap` 类型的方法进行操作
