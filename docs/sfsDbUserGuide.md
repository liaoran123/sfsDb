...
**性能优化提示**：
- **手动释放可显著提高性能**：由于 sfsDb 未实现 finalizer 机制（注：由于需要解决并发问题，实现过于复杂），因此手动调用 `PutMap()` 函数尤为重要
- **确保对象正确归还**：使用完毕的 map 对象必须手动归还给对象池，否则会导致内存泄漏
- **提高对象复用率**：手动归还的对象可以立即被其他操作复用，增加了对象池的命中率
- **减少内存分配**：通过对象复用，减少了频繁创建和销毁 map 对象带来的内存分配开销
- **高频操作场景效果显著**：在需要大量创建和销毁 map 对象的场景下，正确使用对象池可以带来显著的性能提升

**实现说明**：
- 由于并发安全问题的复杂性，sfsDb 未实现基于 finalizer 的自动回收机制
- 目前的实现专注于提供简单有效的对象池管理，确保基本的对象复用功能
- 手动管理模式虽然需要开发者更多关注，但避免了并发问题带来的复杂性和潜在风险
...
m := engine.GetMap()
// 使用 map
defer engine.PutMap(m) // 推荐：使用 defer 确保对象被归还
...
sfsDb 实现了批处理对象池，用于优化批处理操作的内存使用和性能。
...
**配置参数**：
- `MaxBatchPoolSize`：最大批处理对象缓存数量（默认 1000）
- `MaxBatchSize`：批处理对象大小阈值（默认 1MB），超过此值的批处理对象不会被缓存
...
**使用方式**：
- 批处理对象池由系统自动管理，无需手动操作
- 所有批量操作都会自动使用批处理对象池
- 可以通过 `GetBatchPoolStats()` 函数获取批处理池的统计信息

**跟踪功能**：
- 可通过 `EnableBatchPoolTracing` 变量启用跟踪
- 可通过 `BatchPoolTracer` 变量设置自定义跟踪函数
- 跟踪包括获取、放回、丢弃等操作的详细信息
...
- `TotalCreates`：总创建次数
- `TotalDrops`：总丢弃次数
- `CurrentSize`：当前池中对象数量
- `MaxSizeReached`：达到最大容量的次数
...
sfsDb 实现了对象池机制，用于优化 Record 和 Records 对象的内存使用和性能：
...
- 对于性能敏感的场景，优先使用手动释放
- 对于复杂代码，确保正确使用 defer 语句来释放对象

通过对象池机制，sfsDb 在处理大量记录时能够保持更好的性能和内存使用效率。
