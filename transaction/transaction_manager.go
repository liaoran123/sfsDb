...
type TransactionManager struct {
	transactions []TableTransactionInterface // 管理的事务列表
...
func NewTransactionManager(batch storage.Batch) *TransactionManager {
	return &TransactionManager{
		transactions: make([]TableTransactionInterface, 0),
		batch:        batch,
		committed:    false,
	}
}
...
// table: 要添加的表
func (tm *TransactionManager) AddTable(table *engine.Table) (TableTransactionInterface, error) {
	if tm.committed {
		return nil, fmt.Errorf("transaction manager already committed")
	}
	tx, err := NewTableTransactionWithBatch(table, tm.batch)
	if err != nil {
		return nil, err
	}
	// 将事务添加到管理列表
...
func (tm *TransactionManager) GetTransactions() []TableTransactionInterface {
...
func WithTransaction(batch storage.Batch, tables []*engine.Table, fn func(transactions map[*engine.Table]TableTransactionInterface) error) error {
	// 创建事务管理器
	tm := NewTransactionManager(batch)
	transactions := make(map[*engine.Table]TableTransactionInterface)
