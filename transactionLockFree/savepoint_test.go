...
func TestSavepoint(t *testing.T) {
	...
	t.Run("CreateSavepointAndRollback", func(t *testing.T) {
		...
		// 创建保存点
		savepoint, err := txMgr.CreateSavepoint("savepoint1")
		if err != nil {
		...
		// 回滚到保存点
		err = txMgr.RollbackToSavepoint(savepoint)
		if err != nil {
			t.Fatalf("Failed to rollback to savepoint: %v", err)
		}

		// 验证回滚效果
		// 注意：由于我们只重置了Batch和清理了缓存，实际的提交操作还没有执行
		// 所以这里我们需要通过检查缓存来验证
		if len(tableTx.Cache) != 0 {
			t.Errorf("Expected empty cache after rollback, got %d entries", len(tableTx.Cache))
		}
	})
	...
	t.Run("NestedSavepoints", func(t *testing.T) {
		...
		// 创建第一个保存点
		savepoint1, err := txMgr.CreateSavepoint("savepoint1")
		if err != nil {
		...
		// 创建第二个保存点
		_, err = txMgr.CreateSavepoint("savepoint2")
		if err != nil {
	...
	t.Run("CommitAfterSavepoint", func(t *testing.T) {
		...
		// 创建保存点
		_, err = txMgr.CreateSavepoint("savepoint1")
		if err != nil {
			t.Fatalf("Failed to create savepoint: %v", err)
		}
